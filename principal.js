const OPENAI_API_KEY = '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'; // ← Pon tu clave real aquí
let nombre = 'fer'
console.log(nombre);
console.warn(nombre);
...
//const OPENAI_API_KEY = 'tu-api-key-aqui';  // Asegúrate de reemplazarlo con tu propia clave API

async function generarImagen() {
    console.log('aqui estoy')
  try {
    // Hacemos la solicitud POST para generar la imagen con DALL·E 3
    const response = await fetch('https://api.openai.com/v1/images/generations', {
      ...
      body: JSON.stringify({
        prompt: "Una ilustración de el volcan santa maría de quetzaltenango, guatemala",  // Descripción de la imagen
        n: 1,  // Número de imágenes a generar
        size: "1024x1024",  // Tamaño de la imagen (puedes usar 256x256, 512x512 o 1024x1024)
        model: "dall-e-3"  // Usamos el modelo DALL·E 3 para la generación de imágenes
      })
    });

    // Si la respuesta es exitosa, extraemos la URL de la imagen generada
    const data = await response.json();
    const imageUrl = data.data[0].url;  // URL de la imagen generada

    console.log("Imagen generada: ", imageUrl);  // Muestra la URL de la imagen generada

    // Mostrar la imagen en el DOM (por ejemplo, en una página web)
    const imgElement = document.createElement("img");
    imgElement.src = imageUrl;
    imgElement.alt = "Imagen generada por DALL·E 3";
    document.body.appendChild(imgElement);  // Agrega la imagen generada a la página web

  } catch (error) {
    console.error("Error al generar la imagen:", error);  // Muestra el error en caso de fallo
  }
}
